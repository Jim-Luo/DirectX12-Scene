...
VSParticleDrawOut VSParticleDraw(VSParticleIn input)
{
	...
	/*if (input.id == 0)
	{
		output.pos = float3(-300.0f, 300.0f, 0.0f);
	...
	{*/
		output.pos.z = g_bufPosVelo[input.id].pos.z;
		output.pos.x = g_bufPosVelo[input.id].pos.x / 4;
		output.pos.y = abs(g_bufPosVelo[input.id].pos.y) / 2 - 300.0f;

		float mag = g_bufPosVelo[input.id].velo.w / 10;
		output.color = lerp(float4(0.2f, 0.0f, 0.0f, 0.0f), input.color, mag);

		if (abs(g_bufPosVelo[input.id].pos.x) > 100.0f || g_bufPosVelo[input.id].pos.y + 300.0f > 300.0f)
		{
			output.pos.z = g_bufPosVelo[input.id].pos.z / 10;
			output.pos.x = g_bufPosVelo[input.id].pos.x / 4 / 10;
			output.pos.y = abs(g_bufPosVelo[input.id].pos.y) / 2 / 10 - 300.0f;
		}
		if (g_bufPosVelo[input.id].velo.w < 0.3)
		{
